...
[[nodiscard]] constexpr bool has_prefix(std::string_view value, std::string_view prefix) noexcept {
    if(value.size() < prefix.size()) {
        return false;
    }

    for(std::size_t pos{}; pos < prefix.size(); ++pos) {
...
[[nodiscard]] constexpr std::string_view stripped_msvc_type_name(std::string_view value) noexcept {
    constexpr std::string_view prefixes[]{"struct ", "class ", "enum "};
...
[[nodiscard]] constexpr auto stripped_type_name() noexcept {
    ...
    return stripped_msvc_type_name(value);
#else
