...
bool ImGui::BeginComboCustom(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
...
// Old API, prefer using BeginCombo() nowadays if you can.
bool ImGui::ComboCustom(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginComboCustom(label, preview_value, ImGuiComboFlags_None))
        return false;
...
bool ImGui::ComboCustom(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
    const char* p = items_separated_by_zeros;       // FIXME-OPT: Avoid computing this, or at least only when combo is open
    while (*p)
    ...
    bool value_changed = ComboCustom(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
...
bool ImGui::SliderScalarCustom(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImRect frame_bb(window->DC.CursorPos - ImVec2(0, 5), window->DC.CursorPos + ImVec2(0, 5) + ImVec2(w, style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(style.ItemInnerSpacing.x, 0.0f));
    ...
    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2, grab_bb.Min.y + (grab_bb.Max.y - grab_bb.Min.y) / 2), ImGui::GetStyle().FramePadding.y * 4, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 12);
    }
        //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                // OnRender if cursor is vertically visible
